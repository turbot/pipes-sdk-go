...
type UsersApiGetPlanRequest struct {
	ctx        _context.Context
	ApiService *UsersService
	userHandle string
}

func (r UsersApiGetPlanRequest) Execute() (IdentityPlan, *_nethttp.Response, error) {
	return r.ApiService.GetPlanExecute(r)
}

/*
GetPlan Get user billing plan.

Get the current user billing plan.

	@param ctx _context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@param userHandle Specify the handle of the user to retrieve the billing plan for.
	@return UsersApiGetPlanRequest
*/
func (a *UsersService) GetPlan(ctx _context.Context, userHandle string) UsersApiGetPlanRequest {
	return UsersApiGetPlanRequest{
		ApiService: a,
		ctx:        ctx,
		userHandle: userHandle,
...
//
//	@return IdentityPlan
func (a *UsersService) GetPlanExecute(r UsersApiGetPlanRequest) (IdentityPlan, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod  = _nethttp.MethodGet
		localVarPostBody    interface{}
		formFiles           []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "UsersService.GetPlan")
	if err != nil {
		return localVarReturnValue, nil, GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/user/{user_handle}/billing/plan"
	localVarPath = strings.Replace(localVarPath, "{"+"user_handle"+"}", _neturl.PathEscape(parameterToString(r.userHandle, "")), -1)
...
// The SQL where filter you wish to apply to this request. The filter will be parsed and sanitised and checked against the supported columns for this API.
func (r UsersApiListUsageRequest) Where(where string) UsersApiListUsageRequest {
	r.where = &where
	return r
}

// The max number of items to fetch per page of data, subject to a min and max of 1 and 100 respectively. If not specified will default to 25.
func (r UsersApiListUsageRequest) Limit(limit int32) UsersApiListUsageRequest {
	r.limit = &limit
	return r
}

// When list results are truncated, next_token will be returned, which is a cursor to fetch the next page of data. Pass next_token to the subsequent list request to fetch the next page of data.
func (r UsersApiListUsageRequest) NextToken(nextToken string) UsersApiListUsageRequest {
	r.nextToken = &nextToken
	return r
}
...
/*
ListUsage List user usage

List all usage metrics for a user.

	@param ctx _context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@param userHandle The handle of the user to list usage for.
	@return UsersApiListUsageRequest
*/
func (a *UsersService) ListUsage(ctx _context.Context, userHandle string) UsersApiListUsageRequest {
	return UsersApiListUsageRequest{
		ApiService: a,
		ctx:        ctx,
		userHandle: userHandle,
...
func (a *UsersService) ListUsageExecute(r UsersApiListUsageRequest) (ListUsageMetricsResponse, *_nethttp.Response, error) {
	...
	localVarPath := localBasePath + "/user/{user_handle}/usage"
	localVarPath = strings.Replace(localVarPath, "{"+"user_handle"+"}", _neturl.PathEscape(parameterToString(r.userHandle, "")), -1)
