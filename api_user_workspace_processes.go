...
type UserWorkspaceProcessesApiCommandRequest struct {
	ctx             _context.Context
	ApiService      *UserWorkspaceProcessesService
	userHandle      string
	workspaceHandle string
	processId       string
	request         *WorkspaceProcessCommandRequest
}

// The request body for the workspace process command.
func (r UserWorkspaceProcessesApiCommandRequest) Request(request WorkspaceProcessCommandRequest) UserWorkspaceProcessesApiCommandRequest {
	r.request = &request
	return r
}

func (r UserWorkspaceProcessesApiCommandRequest) Execute() (WorkspaceProcessCommandResponse, *_nethttp.Response, error) {
	return r.ApiService.CommandExecute(r)
}

/*
Command Run user workspace process command

Run a command against a users workspace process.

	@param ctx _context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@param userHandle The handle of the user where we want to run the workspace process command.
	@param workspaceHandle The handle of the workspace where the process command will be executed.
	@param processId The id of the process where the command will be executed against.
	@return UserWorkspaceProcessesApiCommandRequest
*/
func (a *UserWorkspaceProcessesService) Command(ctx _context.Context, userHandle string, workspaceHandle string, processId string) UserWorkspaceProcessesApiCommandRequest {
	return UserWorkspaceProcessesApiCommandRequest{
		ApiService:      a,
		ctx:             ctx,
...
//
//	@return WorkspaceProcessCommandResponse
func (a *UserWorkspaceProcessesService) CommandExecute(r UserWorkspaceProcessesApiCommandRequest) (WorkspaceProcessCommandResponse, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod  = _nethttp.MethodPost
		localVarPostBody    interface{}
		formFiles           []formFile
		localVarReturnValue WorkspaceProcessCommandResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "UserWorkspaceProcessesService.Command")
	if err != nil {
		return localVarReturnValue, nil, GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/user/{user_handle}/workspace/{workspace_handle}/process/{process_id}/command"
